...
type ColumnType = ColumnDef<OnboardingInterface, unknown>;
...
export function OnboardingListPage(props: OnboardingListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<OnboardingInterface>>(
    () => `/onboardings?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: OnboardingInterface) => {
    if (hasAccess('onboarding', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/onboardings/view/${row.id}`);
    }
  };
